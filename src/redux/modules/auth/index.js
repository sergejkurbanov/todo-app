...
const authReducer = (state = defaultState, { type, payload }) => {
  switch (type) {
    ...
    case types.SIGNUP_USER_SUCCESS: {
      ...
      return { ...state, isSignupDone: true, isSignupLoading: false }
    }

    case types.SIGNUP_USER_ERROR: {
      toast.error(payload.error.message || 'Error signing up. Try again later.')
      return { ...state, isSignupDone: true, isSignupLoading: false }
    }
    ...
    case types.LOGIN_USER_ERROR: {
      toast.error(payload.error.message || 'Error signing in. Try again later.')
